...
def main():
    print("Loading exported data for APOGEE...")
    ...
    with Pool(processes=CPU_WORKERS_PREPROCESS) as pool:
        X_features_list = list(tqdm(
            ...
            total=total_stars,
            desc="30D APOGEE Feature Extraction",
            unit="star"
        ))

    X_features = np.array(X_features_list, dtype=np.float32)
    np.save(out_path, X_features)
    print(f"Success! APOGEE Feature matrix saved. Shape: {X_features.shape}")
