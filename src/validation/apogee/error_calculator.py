...
def run_real_bulk_evaluation():
    print(f"[APOGEE Eval] Compute device → {DEVICE}\n")
    ...
    if not (os.path.exists(flux_path) and os.path.exists(feature_path) and os.path.exists(label_path)):
        print(f"Error: Processed APOGEE files not found in {proc_dir}")
    ...
    print(f"Loaded {len(val_idx)} validation samples for APOGEE.")
    ...
    for idx in tqdm(range(len(val_idx)), desc="APOGEE Evaluation"):
        ...
        # To tensor
        tensor_flux = torch.from_numpy(norm_flux).float().unsqueeze(0).to(DEVICE) # (1, 3, 2800)
    ...
    print("\n" + "=" * 80)
    print(" APOGEE VALIDATION PERFORMANCE")
    print("=" * 80)
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write("=" * 70 + "\n")
        f.write("  APOGEE Validation Split Evaluation Report\n")
        f.write("=" * 70 + "\n\n")
