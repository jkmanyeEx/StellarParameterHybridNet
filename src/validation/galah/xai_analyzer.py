...
def extract_30d_features_live_eval(wave, norm_flux):
    """
    Delegates to GALAH extract_45d_features_single_star.
    """
    return extract_45d_features_single_star(wave, norm_flux)


# ── 아키텍처 차원 상수 ────────────────────────────────────────────────────────
CNN_BRANCH_DIM   = 6400   # 4 arms x 1600
...
def calculate_per_line_weight_attribution(model):
    """
    Measure each absorption line's contribution through the Dense branch for GALAH.
    Dense branch first layer: Linear(45, 128).
    """
    try:
        first_weight = None
        ...
        results = []
        for i, line_name in enumerate(LINE_NAMES_45D):
            col_s = i * 3
            col_e = col_s + 3
...
def calculate_eval_model_weight_ratio(model):
    ...
    Concat order: [CNN:6400 | Dense:128] = 6528 total.
    """
    try:
        target = None
        for name, param in model.named_parameters():
...
def run_xai_line_profile_analysis(num_samples=100):
    ...
    print(f"[GALAH XAI Engine] Initializing on: {device}")
    ...
    # GALAH Absorption lines mapping to arms
    absorption_lines = {
    ...
    for idx in tqdm(sample_indices, desc="GALAH Jacobian XAI"):
        ...
        norm_flux_t  = torch.from_numpy(norm_flux).float() # (4, 4000)

        # Baseline Jacobian
        input_base = norm_flux_t.unsqueeze(0).to(device).requires_grad_(True) # (1, 4, 4000)
    ...
    # Proof ratio based on CCD3 H-alpha vs non H-alpha in CCD3
    wave_ccd3 = wave_grid[2]
    cont_mask = ~((wave_ccd3 >= 6513) & (wave_ccd3 <= 6613))
    bg         = float(np.mean(mean_jac[0, 2, cont_mask]))
    proof_r    = line_scores["H-alpha (CCD3)"][0] / (bg + 1e-8)

    bg_abl     = float(np.mean(mean_jac_ablated[0, 2, cont_mask]))
    ha_abl     = float(np.mean(mean_jac_ablated[0, 2, (wave_ccd3 >= 6513) & (wave_ccd3 <= 6613)]))
    ...
    print(f"45D branch weight:     {weight_ratio:.2f}%")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("=" * 60 + "\n\n")
        f.write(f"Analyzed Samples: {valid_count} Synthetic/Real GALAH Spectra\n\n")
        ...
        f.write(f"Proof Ratio (Ablated 45D Features): {proof_r_abl:.4f}\n\n")
        f.write("=" * 60 + "\n")
        f.write("▶ Global Weight Attribution Architecture\n")
        f.write("=" * 60 + "\n")
        f.write(f"   - 45D Physical Feature Layer Contribution: {weight_ratio:.4f}%\n\n")
        if per_line:
            f.write("=" * 60 + "\n")
