...
from src.validation.apogee.eval_core import align_wavelength_resolution, read_sdss_spec

BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
APOGEE_FLUX = os.path.join(BASE_DIR, "data", "apogee", "processed", "X_flux_clean.npy")

def main():
    if not os.path.exists(APOGEE_FLUX):
        print(f"[Error] APOGEE training file not found at: {APOGEE_FLUX}")
        return

    apogee = np.load(APOGEE_FLUX)
    print("=" * 70)
    print("APOGEE TRAINING flux (what the model learned on)")
    print("=" * 70)
    print(f"  shape         : {apogee.shape}")
    print(f"  per-spectrum mean (first 3): "
          f"{[f'{np.mean(apogee[i]):.3f}' for i in range(3)]}")
    print(f"  global min/max: {apogee.min():.3f} / {apogee.max():.3f}")
    print(f"  global mean   : {apogee.mean():.3f}")
    print(f"  → continuum-normalized: values oscillate around ~1.0")

    print("\n" + "=" * 70)
    print("Validation spec EVAL flux")
    print("=" * 70)
    ...
    if sample and os.path.exists(sample):
        ...
        aligned = align_wavelength_resolution(loglam, flux, 2800)[0]
        ...
        print("\n" + "=" * 70)
        print("Validation spec flux AFTER continuum normalization (the fix)")
        print("=" * 70)
        safe = np.copy(flux)
        ...
        print(f"  → NOW it oscillates around ~1.0, matching APOGEE!")
    else:
