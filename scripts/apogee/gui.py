"""
APOGEE DR17 Stellar Parameter Evaluation GUI.

Loads the APOGEE validation split from data/apogee/processed/ and displays
per-star predictions, 3-arm spectra, Jacobian XAI sensitivity, and weight
attribution.

APOGEE HERMES 3-arm layout:
  Blue  chip : ~15140–15800 Å
  Green chip : ~15840–16440 Å
  Red   chip : ~16480–16960 Å
"""
...
from src.validation.apogee.xai_analyzer import calculate_per_line_weight_attribution

BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '../..'))

# ── Paths ─────────────────────────────────────────────────────────────────────
_proc_dir    = os.path.join(BASE_DIR, "data", "apogee", "processed")
...
for p in (_flux_path, _label_path, _wave_path, _ls_path, _fs_path):
    if not os.path.exists(p):
        raise FileNotFoundError(
            ...
            "Execute the APOGEE preprocessing and training pipeline first."
        )

WAVE_GRID    = np.load(_wave_path)                       # (3, 2800)
...
# ── APOGEE 3-arm wavelength ranges ────────────────────────────────────────────
ARM_RANGES = [
    (15140, 15800, "Blue chip"),
    (15840, 16440, "Green chip"),
    (16480, 16960, "Red chip"),
]

# Absorption lines per arm (name, arm_idx, center_Å, color)
ABSORPTION_LINES = [
    ("Fe 15648",   0, 15648, "#ff6644"),
    ("Fe 15200",   0, 15200, "#ffaa44"),
    ("Mg 15749",   0, 15749, "#44ffaa"),
    ("Si 15960",   1, 15960, "#44ff44"),
    ("Br-14",      1, 15884, "#ff4444"),
    ("Fe 16040",   1, 16040, "#ffdd44"),
    ("Si 16094",   1, 16094, "#aaff44"),
    ("Br-11",      2, 16811, "#ff8888"),
    ("Si 16680",   2, 16680, "#ff44ff"),
    ("Al 16755",   2, 16755, "#44aaff"),
]

# XAI pixel-index markers built after WAVE_GRID is loaded
XAI_LINE_MARKERS = []
...
class StellarValidatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("APOGEE DR17 Stellar Parameter Evaluation")
    ...
    # ── Model ─────────────────────────────────────────────────────────────────
    def init_model(self):
        weights_path = os.path.join(BASE_DIR, "weights", "apogee",
    ...
    def _load_val_split(self):
        ...
        self.X_flux   = np.array(X_flux_all[val_indices])    # (N, 3, 2800)
    ...
    def generate_physics_description(self, true_val, pred_val,
        ...
        ratio = (f"{learned_wr:.2f}% / {nominal_wr:.2f}% "
                 f"(x{learned_wr/max(nominal_wr,1e-8):.2f})")
        return (
            f" XAI Mode: {mode}\n\n"
            f" Spectral Class\n"
    ...
    def setup_ui(self):
        ...
        tk.Label(left, text="APOGEE DR17 Stellar Evaluation",
                 font=("Helvetica", 13, "bold"),
                 bg="#12121f", fg="#ffffff").pack(pady=12)

        self.path_lbl = tk.Label(
            left, text=f"Evaluation split: {self.n_val} stars",
        ...
        self.idx_lbl = tk.Label(nav, text="--",
        ...
        line_frame = tk.LabelFrame(
            left, text="Per-Line Weight Attribution (30D)",
            font=("Helvetica", 9), bg="#12121f", fg="#aaaaff", padx=6, pady=4)
        ...
        for lname, pct in self.per_line_attr[:10]:
            lbl = tk.Label(line_frame,
                           text=f"{lname:<14s} {pct:5.1f}%",
                           font=("Consolas", 8), bg="#12121f",
                           fg="#ccccff", anchor="w")
            lbl.pack(fill=tk.X)
        ...
        tbl = tk.LabelFrame(left, text="Parameter Comparison",
                            font=("Helvetica", 10), bg="#12121f",
                            fg="#88ff88", padx=8, pady=8)
        ...
        for i, name in enumerate(["T_eff (K)", "log g (dex)", "[Fe/H] (dex)"]):
            tk.Label(tbl, text=name, font=("Helvetica", 10, "bold"),
                     bg="#12121f", fg="#ffffff").grid(
                         row=i+1, column=0, sticky="w", padx=2, pady=8)
            for j, (lst, fg) in enumerate([
                (self.pred_labels,  "#88ffcc"),
    ...
    def toggle_ablated(self):
        ...
        if self.norm_flux_3arm is not None:
    ...
    def update_profile(self):
        ...
        raw_flux_3arm = self.X_flux[self.current_idx]     # (3, 2800)
        real_true     = self.Y_labels[self.current_idx]

        # Per-arm z-score (identical to dataset.py)
        f_mean = np.mean(raw_flux_3arm, axis=1, keepdims=True)
        f_std  = np.std(raw_flux_3arm,  axis=1, keepdims=True) + 1e-8
        self.norm_flux_3arm = np.clip(
            (raw_flux_3arm - f_mean) / f_std, -3.0, 3.0)

        # 30D features
        raw_feat  = extract_30d_features_single_star(WAVE_GRID, raw_flux_3arm)
        norm_feat = (raw_feat - FEATURE_MEAN) / (FEATURE_STD + 1e-8)
        self.physical_features = norm_feat.astype(np.float32)

        t_flux = (torch.from_numpy(self.norm_flux_3arm)
                  .float().unsqueeze(0).to(self.device))   # (1, 3, 2800)
        t_feat = (torch.from_numpy(self.physical_features)
                  .float().unsqueeze(0).to(self.device))   # (1, 30)
        ...
        fmt = [f"{real_pred[0]:.1f} K", f"{real_pred[1]:.3f}",
               f"{real_pred[2]:.3f}"]
        tru = [f"{real_true[0]:.1f} K", f"{real_true[1]:.3f}",
               f"{real_true[2]:.3f}"]
        err = [f"{err_teff:.2f}%", f"{err_logg:.2f}%",
               f"{abs(err_feh):.4f} dex"]
        for i in range(3):
        ...
        self.weight_share_lbl.configure(
            text=f"30D: {learned_wr:.2f}% (nominal {nominal_wr:.2f}%)")
        ...
        for arm_idx, (ax, color) in enumerate(zip(self.ax_arms, arm_colors)):
            ...
            for aname, a_arm, a_center, a_color in ABSORPTION_LINES:
                if a_arm == arm_idx:
                    ax.axvline(a_center, color=a_color,
                               alpha=0.5, linewidth=0.8, linestyle="--")
                    ax.text(a_center,
                            ax.get_ylim()[1] * 0.85 if ax.get_ylim()[1] > 0
                            else 1.0,
                            aname, color=a_color, fontsize=4,
                            ha='center', rotation=90, alpha=0.8)

        self.execute_automated_jacobian()

    def execute_automated_jacobian(self):
        if self.norm_flux_3arm is None:
            return
        ...
        if self.show_ablated:
            t_feat = torch.zeros(
                1, len(self.physical_features), device=self.device)
        ...
        # Flatten (3, 2800) → concat for display
        jac     = np.abs(t_flux.grad.cpu().numpy()[0])   # (3, 2800)
        jac_cat = np.concatenate([jac[i] for i in range(3)])
        ...
        for i in range(1, 3):
            self.ax_xai.axvline(i * 2800, color="#555566",
                                linewidth=0.8, linestyle=":")
        y_top = float(np.max(jac_sm)) if np.max(jac_sm) > 0 else 0.035
        arm_labels = ["Blue chip", "Green chip", "Red chip"]
        for i, al in enumerate(arm_labels):
            self.ax_xai.text((i + 0.5) * 2800, y_top * 0.92,
        ...
        self.ax_xai.set_xlabel(
            "Pixel index (3 arms concatenated)",
