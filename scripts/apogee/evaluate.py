...
from src.validation.apogee.error_calculator import run_real_bulk_evaluation
...
def _resolve_weights(weights_arg, survey_dir):
    ...
    Priority:
      ...
      2. stellar_hybrid_model.pth   (latest pointer written by engine.py).
      3. Most recently modified stellar_hybrid_model_n*.pth in the directory.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate APOGEE StellarParameterHybridNet on the test set"
    )
    parser.add_argument(
    ...
    survey_dir  = os.path.join(base_dir, "weights", "apogee")
