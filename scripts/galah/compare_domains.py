...
from src.validation.galah.eval_core import align_wavelength_resolution, read_sdss_spec

BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
GALAH_FLUX = os.path.join(BASE_DIR, "data", "galah", "processed", "X_flux_clean.npy")

def main():
    if not os.path.exists(GALAH_FLUX):
        print(f"[Error] GALAH training file not found at: {GALAH_FLUX}")
        return

    galah = np.load(GALAH_FLUX)
    print("=" * 70)
    print("GALAH TRAINING flux (what the model learned on)")
    print("=" * 70)
    print(f"  shape         : {galah.shape}")
    print(f"  per-spectrum mean (first 3): "
          f"{[f'{np.mean(galah[i]):.3f}' for i in range(3)]}")
    print(f"  global min/max: {galah.min():.3f} / {galah.max():.3f}")
    print(f"  global mean   : {galah.mean():.3f}")
    ...
    if sample and os.path.exists(sample):
        ...
        aligned = align_wavelength_resolution(loglam, flux, 4000)[0]
        ...
        print(f"  → NOW it oscillates around ~1.0, matching GALAH!")
    else:
