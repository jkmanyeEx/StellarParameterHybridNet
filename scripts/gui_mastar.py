...
class StellarValidatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("MaStar Stellar Parameter Evaluation")
        ...
        self.setup_ui()
        self.root.after(100, self.lazy_load_mastar_data)
    ...
    def lazy_load_mastar_data(self):
        try:
            flux_matrix, label_matrix, valid_paths = load_mastar_spectra()
            self.X_flux           = flux_matrix
            self.Y_labels         = label_matrix
            self.valid_paths      = valid_paths
            self.max_virtual_rows = len(self.Y_labels)
            self.path_lbl.configure(
                text=f"Dataset: {self.max_virtual_rows} MaStar samples loaded")
            self.update_profile()
    ...
    def setup_ui(self):
        ...
        tk.Label(left, text="MaStar Stellar Parameter Evaluation",
        ...
        for col, (txt, fg) in enumerate([
            ("Model Pred","#88ccff"), ("MaStar GT","#ffaaee"), ("Error","#ffffaa")
        ], start=1):
        ...
        for i, name in enumerate(["T_eff (K)", "Log g (dex)", "[Fe/H] (dex)"]):
            ...
            for col, (lst, fg) in enumerate([(self.pred_labels,"#88ffcc"),
                                              (self.true_labels,"#ff88aa"),
                                              (self.error_labels,"#ffff77")], start=1):
                lbl = tk.Label(tbl, text="-", font=("Consolas", 11, "bold"),
                               bg="#12121f", fg=fg)
                lbl.grid(row=i+1, column=col, padx=6)
                lst.append(lbl)
    ...
    def update_profile(self):
        ...
        for i in range(3):
            self.pred_labels[i].configure(
                text=[f"{real_pred[0]:.1f} K", f"{real_pred[1]:.3f}", f"{real_pred[2]:.3f}"][i])
            self.true_labels[i].configure(
                text=[f"{real_true[0]:.1f} K", f"{real_true[1]:.3f}", f"{real_true[2]:.3f}"][i])
            self.error_labels[i].configure(
                text=[f"{err_teff:.2f}%", f"{err_logg:.2f}%", f"{abs(err_feh):.4f} dex"][i])
        ...
        self.ax_spec.plot(WAVE_GRID, self.current_flux[0],
                          ...
                          label=f"MaStar #{self.current_idx + 1}", zorder=3)
        for name, lo, hi, color in ABSORPTION_LINES:
            self.ax_spec.axvspan(lo, hi, color=color, alpha=0.15, zorder=2)
            self.ax_spec.text((lo+hi)/2, 3.85, name, color=color,
                              fontsize=5, ha='center', va='bottom',
                              rotation=90, alpha=0.9)
        self.ax_spec.set_title(f"Stellar Spectrum — MaStar #{self.current_idx + 1}",
    ...
    def execute_automated_jacobian(self):
        ...
        if self.show_ablated:
            t_feat = torch.zeros(1, len(self.physical_features_30d), device=self.device)
